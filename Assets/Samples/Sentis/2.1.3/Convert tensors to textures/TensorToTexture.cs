...
public class TensorToTexture : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        using Tensor<float> tensor = TextureConverter.ToTensor(inputTexture);
        Debug.Log($"tensor:{tensor.shape}");
        TextureConverter.RenderToTexture(tensor, outputRenderTexture);
        Debug.Log($"RT:{outputRenderTexture.height},{outputRenderTexture.width},{outputRenderTexture.format}");
